...
class CrossAssetGRUAttention(nn.Module):
    ...
    def __init__(
        ...
        # --- Stage 1: Per-asset temporal encoder (shared GRU) ---
        ...
        # --- Stage 2: Cross-Asset Self-Attention ---
        # Learnable asset embeddings (captures asset identity)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # project: (B, A, T, F) → (B, A, T, D)
        h: Tensor = F.gelu(self.input_norm(self.input_proj(x)))

        # reshape for shared GRU: (B*A, T, D)
        h = h.reshape(B * A, T, D)
        gru_out, _ = self.gru(h)  # (B*A, T, D)

        # take last hidden state → (B*A, D)
        h = self.temporal_norm(gru_out[:, -1, :])

        # --- Stage 2: Cross-Asset Attention ---
        # reshape: (B, A, D)
        h = h.reshape(B, A, D)
        # add asset identity embeddings
        h = h + self.asset_embed[:, :A, :]
        # self-attention over asset dimension
    ...
    def get_attention_weights(self, x: Tensor) -> Tensor:
        ...
        h = h + self.asset_embed[:, :A, :]
        # manually compute attention weights from first cross-attention layer
        layer = self.cross_attention.layers[0]
        # self_attn is the MultiheadAttention module
