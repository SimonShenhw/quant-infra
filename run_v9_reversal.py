# ============================================================================
# HISTORICAL ARTIFACT / 历史存档 - v9: reversal diagnosis (proved model > pure factors)
...
# ============================================================================
"""
run_v9_reversal.py — Reversal-based cross-sectional ranking.

Key insight from v8 diagnosis: all factor ICs are NEGATIVE at 1h.
Crypto at 1h is a mean-reversion market, not momentum.
Fix: flip the ranking direction — SHORT recent winners, LONG recent losers.
"""
        ...
        with torch.no_grad():
            for t in range(cursor, oos_end):
                ...
                if current_long < 0:
                    ...
                    # REVERSAL: long the model's LOWEST score (recent loser), short the HIGHEST (recent winner)
                    nl, ns = scores.argmin().item(), scores.argmax().item()
                    if nl != current_long or ns != current_short:
                        need_rebal = True

                if need_rebal and (hold_counter >= min_hold or current_long < 0):
                    nl = scores.argmin().item()   # REVERSAL: long the lowest-scored
                    ns = scores.argmax().item()   # REVERSAL: short the highest-scored
                    cost_bar = 0.0
...
def main():
    print("=" * 65)
    print("  v9.0 — REVERSAL | 1M+ Bars | WFO | GRU+CrossAttn")
    print("=" * 65)
    ...
    print("\n" + "=" * 65)
    print("  v9.0 REVERSAL BACKTEST (1M+ bars, WFO, TWAP+AdverseSelection)")
    print("=" * 65)
